...
(function () {
        ...
        async function loadNextPosts() {
            ...
            try {
                ...
                if (newArticles.length > 0) {
                    ...
                    // Sync bookmarks for newly appended cards
                    if (window.syncBookmarkButtons) {
                        window.syncBookmarkButtons();
                    }
    ...
    /**
     * 11. Bookmarks & Reading List (Save for Later)
     */
    function initBookmarks() {
        const drawer = document.getElementById('bookmarks-drawer');
        const toggleBtn = document.getElementById('bookmarks-toggle-btn');
        ...
        const headerBadge = document.getElementById('header-bookmarks-count');
        const itemsList = document.getElementById('bookmarks-items-list');
        const emptyState = document.getElementById('bookmarks-empty-state');
        const totalCounter = document.getElementById('bookmarks-total-counter');
        const clearAllBtn = document.getElementById('bookmarks-clear-all');

        function getBookmarks() {
            try {
                return JSON.parse(localStorage.getItem('custom_theme_bookmarks') || '[]');
            } catch (e) {
                return [];
            }
        }
        ...
        function setBookmarks(bookmarks) {
            localStorage.setItem('custom_theme_bookmarks', JSON.stringify(bookmarks));
        ...
        function isBookmarked(postId) {
            ...
            return list.some(b => String(b.id) === String(postId));
        }

        function toggleBookmark(data) {
            let list = getBookmarks();
            const index = list.findIndex(b => String(b.id) === String(data.id));
        ...
        function updateBadgeAndButtons() {
            const list = getBookmarks();
            ...
            document.querySelectorAll('.btn-bookmark').forEach(btn => {
                ...
                btn.setAttribute('title', saved ? 'Remove from saved stories' : 'Save story for later');
            });
        }
        ...
        function renderDrawerItems() {
            ...
            if (totalCounter) {
                totalCounter.textContent = count === 1 ? '1 story' : count + ' stories';
            }
            ...
            itemsList.innerHTML = list.map(item => `
                <div class="bookmark-item" data-post-id="${item.id}">
                    ${item.thumb ? `
                        <a href="${item.url}" class="bookmark-item-thumb">
                            <img src="${item.thumb}" alt="${item.title}" loading="lazy">
                        </a>
                    ` : ''}
                    <div class="bookmark-item-body">
                        ${item.category ? `<span class="bookmark-item-category">${item.category}</span>` : ''}
                        <h4 class="bookmark-item-title">
                            <a href="${item.url}">${item.title}</a>
                        </h4>
                        ${item.readingTime ? `<span class="bookmark-item-meta">${item.readingTime}</span>` : ''}
                    </div>
                    <button type="button" class="bookmark-item-remove" data-remove-id="${item.id}" aria-label="Remove story" title="Remove story">
        ...
        window.syncBookmarkButtons = updateBadgeAndButtons;
        ...
        document.addEventListener('click', function (e) {
            ...
            if (bookmarkBtn) {
                ...
                const data = {
                    id: bookmarkBtn.getAttribute('data-post-id'),
                    title: bookmarkBtn.getAttribute('data-title'),
                    url: bookmarkBtn.getAttribute('data-url'),
                    thumb: bookmarkBtn.getAttribute('data-thumb'),
                    category: bookmarkBtn.getAttribute('data-category'),
                    readingTime: bookmarkBtn.getAttribute('data-reading-time')
                };

                toggleBookmark(data);

                bookmarkBtn.classList.add('bookmark-bouncing');
                setTimeout(() => bookmarkBtn.classList.remove('bookmark-bouncing'), 400);
                return;
            }
            ...
            if (removeBtn) {
                ...
                let list = getBookmarks().filter(b => String(b.id) !== String(removeId));
                setBookmarks(list);
                return;
            }
        });
        ...
        if (clearAllBtn) {
            clearAllBtn.addEventListener('click', function (e) {
                e.preventDefault();
                if (confirm('Clear all saved stories from your reading list?')) {
                    setBookmarks([]);
                }
            });
        }
